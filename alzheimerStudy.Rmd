...
We can use this latest ML_ready_250, or machine learning ready table of 250 genes in common most/least expressed in AD females with the classifications (23) using a fast and relatively new ML software called H2O.

A note on H2O: it is faster than most machine learning data sets and is used as a better compression format for very large datasets, while being an extension to Principal Component Analysis (PCA) in extracting the PCAs with the most variance to analyze. These commands don't take very long to run.

[H20 tutorial](http://docs.h2o.ai/h2o-tutorials/latest-stable/tutorials/glm/glm.html)
Installed [java for 64bit ](https://docs.oracle.com/en/java/javase/13/index.html) when R didn't run
...
This model doesn't use regularization of the default by setting lambda=0, 
regularization generalizes better between the training set and test and validation set
by reducing the over and under fit of model.
```{r}
...
Get confusion matrix in the training data.
```{r}
h2o.confusionMatrix(m2, valid=FALSE) 
```

Get confusion matrix in the validation data.
```{r}
...
***
#### Binary ML ready data set
Lets go back to the ML_ready_250 data set and limit the classes to two, so that we can use binomial classification on Healthy or Disease. For this purpose the overweight females will be diseased because many health problems associated with being overweight.
```{r}
...
Now, lets use H2O for binary classification and see results produced for the confusion matrix.
```{r}
h2o.init(nthreads=-1, max_mem_size="2G")
...
With binary classification the error rate dropped from 80-90% on 23 classes to 29-30% on 2 classes.


***

Lets use some of the other machine classifiers on this data of binary values and the data of 23 classes.
```{r, message=FALSE, error=FALSE, warning=FALSE}
library(caret)
...
Combined Models, similar to voting in ensemble methods of best algorithm score.
```{r}
...
The Random Forest algorithm, with differences by default than the random forest model in the caret package used previously.
```{r}
...
Now with the binary classification set, ML_ready_250_binomial_class. When predicting with 23 classes, the accuracy above isn't much better than half and some models couldn't be used like LDA.
...
***
From the above, using the binary class system of predictions with various models, the accuracy jumped to 70-83% accuracy with Random Forest of the caret package as the Random Forest package failed for the structure of this binary data and was commented out. The next best model was glm or generalized linear machines with 81% accuracy.

***

***

I want to go back to the data on the hemochromatosis and compare to healthy and flu as well as tetanis vaccinated samples. Create a data set of the top 20 genes most and least expressed in hemochromatosis from the all_6_studies.csv, and the Dracula.csv better named sample types/classes, to create an ML ready data set of 20 genes to classify with H2O, random forests, glm, rpart, LDA, KNN, and so on as done above.
```{r}
...
Sort the all table by hemochromatosis by hemo_FC_malesOverall and hemo_FC_femalesOverall.
```{r}
...
Class_Type <- gsub('-','_', colnames(IronData3))
...
***
***
The file we want to use ML on is now saved as hemochromatosis_ML_ready.csv from the IronData_ML data table. 
```{r}
...
The above H2O commands ran very fast. Even with 23 classes on a selected few features of a smaller subset of genes that were the most or least unique genes in the female and male datasets of fold change in hemochromatosis patients' blood gene expression profiles, the accuracy in prediction is terrible. H2O is an extenstion of Principal Component Analysis and only keeps the components or features with the most variation in the data. We should shorten this data of 23 classes to something more reasonable. Also, GLM in R can use only binary or 2 features with the settings used. This will be left to determine which classes to fit these 23 current class types into for better accuracy in prediction. The flu samples, combine the mixed gender, make one overweight class, one healthy class, etc.

For now, I am just curious about how immunizations for flu and tetanis react in hemochromatosis people.
```{r}
...
#This next line has to be done so the original factors of 23 levels isn't included in the output ML results
FHT_HemoOnly$Class_Type <- as.factor(as.character(FHT_HemoOnly$Class_Type))
unique(FHT_HemoOnly$Class_Type)
```

```{r}
colnames(FHT_HemoOnly) <- gsub('-', '.', colnames(FHT_HemoOnly))
...
Using K-Nearest Neighbor, the prediction accuracy improved from roughly 36% using 23 classes, to 67% using 12 classes, but we can do better by examining those sample IDs again.
...
The Random Forest algorithm from the RandomForest R package scored the same as the KNN algorithm with the caret package. And using 12 levels of classes to classify.


Now, lets change the flu non immunized to 'healthy', the flu with antibiotics to its own class, the flu without antibiotics to its own class, combine the Hemo males and females, and keep the tetanis immunized as its own class. Then see how well the prediction accuracy improves.
```{r}
Hemo2 <- FHT_HemoOnly[grep('Hemo', FHT_HemoOnly$Class_Type),]
Hemo2$Class_Type <- 'Hemochromatosis'
FluAntb <- FHT_HemoOnly[grep('Flu Immunized Plus', FHT_HemoOnly$Class_Type),]
FluAntb$Class_Type <- 'Flu Vaccine and Antibiotics'
FluOnly <- FHT_HemoOnly[grep('Flu Immunized Only', FHT_HemoOnly$Class_Type),]
FluOnly$Class_Type <- 'Flu vaccinated only'
healthy <- FHT_HemoOnly[grep('healthy', FHT_HemoOnly$Class_Type),]
healthy$Class_Type <- 'healthy non-immunized 18-45 year olds'
tetn <- FHT_HemoOnly[grep('Tetan', FHT_HemoOnly$Class_Type),]  

NewDataHemo_ML <- rbind(Hemo2,FluAntb,FluOnly,healthy,tetn)
unique(NewDataHemo_ML$Class_Type)
```

So now there are five classes to predict if a blood sample was healthy non-vaccinated, flu vaccinated, flu vaccinated with antibiotics, hemochromatosis, or tetanis immunized. Lets see how well KNN and random forest do with fewer classes to predict. The last results were 67% with 12 classes, and 38% with 23 classes for KNN.


```{r}
inTrain <- createDataPartition(y=NewDataHemo_ML$Class_Type, p=0.7, list=FALSE)

trainingSet <- NewDataHemo_ML[inTrain,]
testingSet <- NewDataHemo_ML[-inTrain,]

set.seed(123)
write.csv(NewDataHemo_ML,'NewDataHemo_ML.csv', row.names=TRUE)
```
...
Using K-Nearest Neighbor, the prediction accuracy improved from roughly 38% using 23 classes, to 67% using 12 classes, and to 92% using 5 classes. 

***

The Random Forest algorithm, with differences by default than the random forest model in the caret package used previously.
```{r}
trainingSet$Class_Type <- as.factor(trainingSet$Class_Type)
testingSet$Class_Type <- as.factor(testingSet$Class_Type)
rf2 <- randomForest(Class_Type~., data=trainingSet)
...
We can see that the models were improved significantly in predicting the five classes from 12 and from 23 classes using the same KNN and random forest R algorthims. The KNN jumped from 67% to 92% and the random forest increased from 67% to 92% as well. The data had 54 observations and 74 features in total, which were the genes found with the least and most expressed using fold change in the males and females with hemochromatosis and filtering for unique genes.
...
D = h2o.importFile(path = normalizePath("NewDataHemo_ML.csv"))
#h2o.summary(D) would print the summary stats of these 251 features
...
Using the H2O GLM algorithm, for error accuracy, the flu and hemochromatosis samples produced 100% accuracy, but the tetanis and healthy non-immunized had error rates of 33% for tetanis immunized in this training and validation set, and 100% error for the healthy non-immunized samples. This GLM model had an error of 2/11. This gave the H20 accuracy in prediction with five classes an 82% accuracy.
```{r}
2/11
9/11
```
